...
from .extra_dyn import driveinertiaterm, frictionforce


def Adj(G, g):
    R = G[0:3, 0:3]
    p = G[0:3, 3]
    return (R.row_join(zeros(3))).col_join((skew(p) * R).row_join(R)) * g


def Adjdual(G, g):
    R = G[0:3, 0:3]
    p = G[0:3, 3]
    return ((R.row_join(zeros(3))).col_join((skew(p) * R).row_join(R))).transpose() * g


def adj(g, h):
    wg = g[0:3, 0]
    vg = g[3:6, 0]
    return (skew(wg).row_join(zeros(3))).col_join((skew(vg)).row_join(skew(wg))) * h


def adjdual(g, h):
    wg = g[0:3, 0]
    vg = g[3:6, 0]
    return ((skew(wg).row_join(zeros(3))).col_join((skew(vg)).row_join(skew(wg)))).transpose() * h
...
def rne_park_forward(rbtdef, geom, ifunc=None):
    ...
    for i in range(rbtdef.dof):
        ...
        V[i] = ifunc(Adj(geom.Tdh_inv[i], V[i - 1])) + ifunc(geom.S[i] * rbtdef.dq[i])
        V[i] = ifunc(V[i])

        dV[i] = (
            ifunc(geom.S[i] * rbtdef.ddq[i])
...
def rne_park_backward(rbtdef, geom, fw_results, ifunc=None):
    ...
    for i in range(rbtdef.dof - 1, -1, -1):
        ...
        Llm = (rbtdef.L[i].row_join(skew(rbtdef.l[i]))).col_join((-skew(rbtdef.l[i])).row_join(eye(3) * rbtdef.m[i]))

        F[i] = Adjdual(Tdh_inv[i + 1], F[i + 1]) + Llm * dV[i] - adjdual(V[i], Llm * V[i])
        F[i] = ifunc(F[i])
