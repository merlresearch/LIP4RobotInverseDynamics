...
from .rne import coriolismatrix, coriolisterm, frictionterm, gravityterm, inertiamatrix, rne
...
class Dynamics:
    ...
    def calc_base_parms(self, regressor_func):
        ...
        Pb, Pd, Kd = find_dyn_parm_deps(self.dof, self.n_dynparms, regressor_func)
        ...
        self.base_idxs = (
            (numpy.matrix([[i for i in range(self.n_dynparms)]]) * numpy.matrix(Pb))
            .astype(float)
            .astype(int)
            .tolist()[0]
        )
