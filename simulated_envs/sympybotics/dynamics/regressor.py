...
from .rne import rne_backward, rne_forward
...
def regressor(rbtdef, geom, ifunc=None):
    ...
    for p, parm in enumerate(dynparms):
        ...
        for i in range(rbtdef.dof):
            rbtdeftmp.Le[i] = list(map(lambda x: 1 if x == parm else 0, rbtdef.Le[i]))
            rbtdeftmp.l[i] = Matrix(rbtdef.l[i]).applyfunc(lambda x: 1 if x == parm else 0)
