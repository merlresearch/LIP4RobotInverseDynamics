...
def rne_khalil_forward(rbtdef, geom, ifunc=None):
    ...
    for i in range(rbtdef.dof):
        ...
        dw[i] = geom.Rdh[i].T * dw[i - 1] + ns * (rbtdef.ddq[i] * z + w_pj.cross(rbtdef.dq[i] * z).reshape(3, 1))
        dw[i] = ifunc(dw[i])
...
def rne_khalil_backward(rbtdef, geom, fw_results, ifunc=None):
    ...
    for i in range(rbtdef.dof - 1, -1, -1):
        ...
        M[i] = (
            rbtdef.L[i] * dw[i]
            + w[i].cross(rbtdef.L[i] * w[i]).reshape(3, 1)
        ...
        m[i] = M[i] + Rdh[i + 1] * m[i + 1] + pdh[i + 1].cross(f_nj).reshape(3, 1)  # + m_e[i]
        m[i] = ifunc(m[i])
