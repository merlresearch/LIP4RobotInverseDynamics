...
def code_to_string(code, out_parms, printer, indent="", realtype="", line_end="", outidxoffset=0):
    ...
    for i in range(len(code[0])):
        codestr += indent + realtype + printer(code[0][i][0]) + " = " + printer(code[0][i][1]) + line_end + "\n"

    for c, out in enumerate(out_parms):
        codestr += "\n"
        for i in range(len(code[1][c])):
            codestr += indent + out + "[" + str(i + outidxoffset) + "] = " + printer(code[1][c][i]) + line_end + "\n"
...
def gen_py_func(code, out_parms, func_parms, func_name="func"):
    ...
    mainpycode = code_to_string(code, out_parms, sympy.printing.lambdarepr.lambdarepr, indent)
...
def gen_julia_func(code, out_parms, func_parms, func_name="func"):
    ...
    code = code[0], [(e.T if isinstance(e, sympy.MatrixBase) else e) for e in code[1]]

    mainccode = code_to_string(code, out_parms, _juliacode, indent, "", "", outidxoffset=1)
