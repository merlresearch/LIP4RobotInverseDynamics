...
_standard_dh_transfmat = sympy.Matrix(
    [
        [
            ...
            _sin(_dh_theta) * _sin(_dh_alpha),
            _dh_a * _cos(_dh_theta),
        ],
        [
            _sin(_dh_theta),
            _cos(_dh_theta) * _cos(_dh_alpha),
            -_cos(_dh_theta) * _sin(_dh_alpha),
...
_modified_dh_transfmat = sympy.Matrix(
    [
        ...
        [
            ...
            _cos(_dh_theta) * _cos(_dh_alpha),
            -_sin(_dh_alpha),
            -_sin(_dh_alpha) * _dh_d,
        ],
        [
            _sin(_dh_theta) * _sin(_dh_alpha),
            _cos(_dh_theta) * _sin(_dh_alpha),
            _cos(_dh_alpha),
            _cos(_dh_alpha) * _dh_d,
        ],
        [0, 0, 0, 1],
    ]
)
...
class RobotDef:
    ...
    def description(self):
        ...
        for i, dh in enumerate(self.dh_parms):
            s += "      %2i  - " % (i + 1) + sstr(dh).replace("(", "").replace(")", "") + "\n"
    ...
    def _gen_symbols(self):
        ...
        # ]
        self.q = sympy.Matrix([[_new_sym("q" + str(i + 1))] for i in range(self.dof)])
        self.dq = sympy.Matrix([[_new_sym(r"\dot{q}_" + str(i + 1))] for i in range(self.dof)])
        self.ddq = sympy.Matrix([[_new_sym(r"\ddot{q}_" + str(i + 1))] for i in range(self.dof)])

        self.non_latex_symbols = {}
        for i in range(self.dof):
            self.non_latex_symbols[self.q[i]] = r"q" + str(i + 1)
            self.non_latex_symbols[self.dq[i]] = r"dq" + str(i + 1)
            self.non_latex_symbols[self.ddq[i]] = r"ddq" + str(i + 1)
    ...
    def _set_dh_parms(self, dh_parms_list):
        ...
        if len(dh_parms_list) != self.dof:
            raise Exception(
                "RobotDef.set_geometry(): provided number of links"
                "differ from robot dof (%d vs %d)." % (len(dh_parms_list), self.dof)
            )
        ...
        for i in range(self.dof):
            ...
            if len(dh_parms_list[i]) != 4:
                raise Exception(
                    "RobotDef.set_dh_parms: wrong number of Denavit-Hartenberg" "parameters (must be 4 per link)."
                )
