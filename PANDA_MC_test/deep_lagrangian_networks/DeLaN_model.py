...
class LagrangianLayer(nn.Module):
    def __init__(self, input_size, n_dof, activation="ReLu"):
        ...
        if activation == "ReLu":
            ...
            raise ValueError(
                "Activation Type must be in ['Linear', 'ReLu', 'SoftPlus', 'Cos'] but is {}".format(self.activation)
            )
...
class DeepLagrangianNetwork(nn.Module):
    def __init__(self, n_dof, **kwargs):
        ...
        if self._w_init == "xavier_normal":
            ...
            raise ValueError(
                "Weight Initialization Type must be in ['xavier_normal', 'orthogonal', 'sparse'] but is {}".format(
                    self._w_init
                )
            )
    ...
    def _dyn_model(self, q, qd, qdd):
        ...
        T = 1.0 / 2.0 * torch.matmul(qd_4d.transpose(dim0=2, dim1=3), H_qd.view(-1, 1, self.n_dof, 1)).view(-1)
