...
from DeLaN_model import DeepLagrangianNetwork
from replay_memory import PyTorchReplayMemory
from utils import init_env, load_dataset
...
if __name__ == "__main__":
    ...
    print("\n\n################################################")
    print("Characters:")
    print("   Test Characters = {}".format(test_labels))
    print("  Train Characters = {}".format(train_labels))
    print("# Training Samples = {:05d}".format(int(train_qp.shape[0])))
    print("")
    ...
    # Save the Model:
    if save_model:
        torch.save({"epoch": epoch_i, "hyper": hyper, "state_dict": delan_model.state_dict()}, "data/delan_model.torch")
    ...
    print("\nPerformance:")
    print("                Torque MSE = {:.3e}".format(err_tau))
    print("              Inertial MSE = {:.3e}".format(err_m))
    print("Coriolis & Centrifugal MSE = {:.3e}".format(err_c))
    print("         Gravitational MSE = {:.3e}".format(err_g))
    print("    Power Conservation MSE = {:.3e}".format(err_dEdt))
    print("      Comp Time per Sample = {:.3e}s / {:.1f}Hz".format(t_eval, 1.0 / t_eval))
    ...
    fig.canvas.set_window_title("Seed = {}".format(seed))

    legend = [mp.patches.Patch(color=color_i[0], label="DeLaN"), mp.patches.Patch(color="k", label="Ground Truth")]
