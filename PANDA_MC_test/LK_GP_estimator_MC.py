...
p.add_argument("-verbose", type=bool, default=False, help="if True plot are generated and scores are printed")
...
m_names = ["m_" + joint_name for joint_name in joint_names]
c_names = ["c_" + joint_name for joint_name in joint_names]
g_names = ["g_" + joint_name for joint_name in joint_names]
...
if flg_frict:
    ...
    if friction_model == "linear":
        ...
        # f_phi_friction = Models_Lagrangian_kernel.f_phi_friction_basic_with_offset
        # init_param_dict['K_friction_par_init'] = np.ones([num_dof,3])
        # init_param_dict['flg_train_friction_par'] = True
...
# move model to cpu and set the cpu as device
...
with torch.no_grad():
    ...
    rk_K = np.linalg.matrix_rank(K.detach().cpu().numpy(), tol=1e-6)
...
# ######### GET TORQUE DECOMPOSITION ##########
if verbose:
    print("\n----- GET TORQUE DECOMPOSITION -----")
with torch.no_grad():
    m_tr_hat = m.get_m_estimates(
        torch.tensor(input_tr[::downsampling], dtype=dtype, device=device),
        torch.tensor(input_tr, dtype=dtype, device=device),
        alpha_tr,
    )
    c_tr_hat = m.get_c_estimates(
        torch.tensor(input_tr[::downsampling], dtype=dtype, device=device),
        torch.tensor(input_tr, dtype=dtype, device=device),
        alpha_tr,
    )
    g_tr_hat = m.get_g_estimates(
        torch.tensor(input_tr[::downsampling], dtype=dtype, device=device),
        torch.tensor(input_tr, dtype=dtype, device=device),
        alpha_tr,
    )
    ...
    m_test1_hat = m.get_m_estimates(
        torch.tensor(input_tr[::downsampling], dtype=dtype, device=device),
        torch.tensor(input_test1, dtype=dtype, device=device),
        alpha_tr,
    )
    c_test1_hat = m.get_c_estimates(
        torch.tensor(input_tr[::downsampling], dtype=dtype, device=device),
        torch.tensor(input_test1, dtype=dtype, device=device),
        alpha_tr,
    )
    g_test1_hat = m.get_g_estimates(
        torch.tensor(input_tr[::downsampling], dtype=dtype, device=device),
        torch.tensor(input_test1, dtype=dtype, device=device),
        alpha_tr,
    )
...
# get true values
m_tr = data_frame_tr[m_names].to_numpy().reshape(-1, num_dof)
c_tr = data_frame_tr[c_names].to_numpy().reshape(-1, num_dof)
g_tr = data_frame_tr[g_names].to_numpy().reshape(-1, num_dof)

m_test1 = data_frame_test1[m_names].to_numpy().reshape(-1, num_dof)
c_test1 = data_frame_test1[c_names].to_numpy().reshape(-1, num_dof)
g_test1 = data_frame_test1[g_names].to_numpy().reshape(-1, num_dof)
...
d_test1["m_hat"] = m_test1_hat
d_test1["c_hat"] = c_test1_hat
d_test1["g_hat"] = g_test1_hat
...
# plot MSE
if verbose:
    Project_Utils.plot_list(
        [err_M_tr_, err_M_test1_], ["Training inertias MSE", "Test inertias MSE"], ["Training", "Test"]
    )
# plt.show()
...
if flg_frict:
    ...
    # tau_frict = m.get_friction_torques(X_tr=torch.tensor(input_tr[::downsampling], dtype=dtype, device=device),
    #                                    X_test=torch.tensor(input_test1, dtype=dtype, device=device),
    #                                    alpha=alpha_tr)

d_tr["T"] = Y_T_tr
d_tr["T_hat"] = Y_T_tr_hat
d_tr["U"] = U_true_tr
d_tr["U_hat"] = Y_U_tr_hat
d_tr["L"] = L_true_tr
d_tr["L_hat"] = Y_L_tr_hat

d_test1["T"] = Y_T_test1
d_test1["T_hat"] = Y_T_test1_hat
d_test1["U"] = U_true_test1
d_test1["U_hat"] = Y_U_test1_hat
d_test1["L"] = L_true_test1
d_test1["L_hat"] = Y_L_test1_hat

if flg_save_trj:
    print("\nSaving results to " + estimate_test1_saving_path)
    pkl.dump([d_test1, M_test1, M_test1_hat_], open(estimate_test1_saving_path, "wb"))
